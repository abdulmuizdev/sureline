...
class CollectionsSearchDao extends DatabaseAccessor<AppDatabase> with _$CollectionsSearchDaoMixin {
  ...
  Future<List<Quote>> getSearchOfCollection(int collectionId, {required bool isPremium}) {
    final query = select(quotes)..where(
      (tbl) => tbl.id.isInQuery(
        selectOnly(collectionsSearchQuotes)
  ...
  Future<void> addCollectionSearch(CollectionsSearchQuotesCompanion collectionSearchQuote) {
  ...
  Future<void> removeCollectionSearch(int collectionId, int quoteId) {
    return (delete(collectionsSearchQuotes)
