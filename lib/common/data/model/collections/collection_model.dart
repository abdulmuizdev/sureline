...
class CollectionModel extends CollectionEntity {
  ...
  factory CollectionModel.fromJson(Map<String, dynamic> json) {
    ...
    if (json['quotes'] != null && json['quotes'] is List) {
      ...
      if (quotesList.isNotEmpty) {
        quotes =
            quotesList
                .map((quote) => FavouriteModel.fromJson(quote as Map<String, dynamic>))
                .toList();
      }
    }
  ...
  Map<String, dynamic> toJson() {
    return {
      'name': name,
      'favouriteQuotes':
          favouriteQuotes.map((quote) => FavouriteModel.fromEntity(quote).toJson()).toList(),
      'ownQuotes': ownQuotes.map((quote) => OwnQuoteModel.fromEntity(quote).toJson()).toList(),
      'historyQuotes':
          historyQuotes.map((quote) => HistoryModel.fromEntity(quote).toJson()).toList(),
      'searchQuotes': searchQuotes.map((quote) => SearchModel.fromEntity(quote).toJson()).toList(),
    };
  }
  ...
  factory CollectionModel.fromEntity(CollectionEntity entity) {
    return CollectionModel(
      id: entity.id,
      name: entity.name,
      favouriteQuotes: entity.favouriteQuotes.map((e) => FavouriteModel.fromEntity(e)).toList(),
      ownQuotes: entity.ownQuotes.map((e) => OwnQuoteModel.fromEntity(e)).toList(),
      historyQuotes: entity.historyQuotes.map((e) => HistoryModel.fromEntity(e)).toList(),
      searchQuotes: entity.searchQuotes.map((e) => SearchModel.fromEntity(e)).toList(),
    );
  }
